  ...
  return (
    <section className="home">
        ...
        <div className="mainpost__container">
            ...
            <h1 className="mainpost__title">Global Climate Summit <br /> Urges Immediate Action</h1>
            <h5 className="mainpost__subtitle">Leaders from around the world gathered for a global climate summit, emphasizing the urgent need for coordinated action to address climate change.</h5>
        ...
        <Grid2 container wrap="nowrap" className="posts__container"> 
            {posts?.map((post)=>(
                <Box key={post.id} className="post" onClick={() => {isLogedIn ? navigate('/post', {state: post}) : navigate('/sign-in')}}>
                    <Skeleton key={post.userId} variant="rectangular" className="img__post"/>
